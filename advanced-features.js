...
// The Selling UI uses FileUploadBuilder#setFileTypes(), but that helper is not
// available in the installed 14.27.0 build on Railway. Keep the fluent API
// compatible so checkout/order modals can still be created. Discord validates
// the upload component itself; uploaded references are stored as attachment URLs.
...
const ADVANCED_COMMANDS = new Set(['panelbuilder', 'configcenter', 'library', 'setupdiff', 'systemcheck']);
...
function buildAdvancedCommandDefinitions() {
  return [
    {
      name: 'panelbuilder',
      description: 'Erstellt und verwaltet eigene Discord-Panels.',
      type: 1,
      options: [
        {
          type: 1,
    ...
    {
      name: 'configcenter',
      description: 'Zentrale Einstellungen für die erweiterten Bot-Funktionen.',
      type: 1,
      options: [
    ...
    {
      name: 'setupdiff',
      description: 'Vergleicht den aktuellen Server mit einem gespeicherten Setup-Snapshot.',
      type: 1,
      options: [
...
async function handlePanelModal(interaction) {
  ...
  if (!pending || pending.expiresAt < Date.now()) return replyEphemeral(interaction, '❌ Dieses Panel-Formular ist abgelaufen. Bitte `/panelbuilder create` erneut ausführen.');
...
async function handleSetupDiff(interaction) {
  ...
  if (!data.setupSnapshot) return replyEphemeral(interaction, '❌ Noch kein Snapshot vorhanden. Nutze zuerst `/setupdiff save`.');
...
async function advancedInteractionHandler(interaction, client) {
  try {
    ...
    if (!interaction.isChatInputCommand?.() || !ADVANCED_COMMANDS.has(interaction.commandName)) return;

    if (interaction.commandName === 'panelbuilder') return handlePanelBuilder(interaction);
    if (interaction.commandName === 'configcenter') return handleConfigCenter(interaction);
    if (interaction.commandName === 'library') return handleLibrary(interaction);
    if (interaction.commandName === 'setupdiff') return handleSetupDiff(interaction);
    if (interaction.commandName === 'systemcheck') return handleSystemCheck(interaction, client);
  } catch (error) {
...
function installAdvancedFeatures(client) {
  ...
  nativeClientOn.call(client, Events.ClientReady, () => {
    console.log('✅ Turbo Advanced Features v5.9.1 aktiv: Panel Builder, Config Center, Library, Setup-Diff, Systemcheck');
